...
def _read_connected_source_entities(
    ...
    for item in objects:
        ...
        # across source kinds.
        entity_name = re.sub(
            r"\.(csv|xlsx|xlsm|txt)$", "", obj_name, flags=re.I
        )
...
def run_discovery(data: dict[str, Any]) -> dict[str, Any]:
    ...
    # CSV folder source
        ...
        source_entities = [
            _read_csv_entity(path)
            for path in sorted(source_path.glob("*.csv"))
        ]

        if not source_entities:
            raise ValueError("No CSV source entities found")
    ...
    # Compiled once per (rules document, source schema) and then applied
    # deterministically to every row below.
