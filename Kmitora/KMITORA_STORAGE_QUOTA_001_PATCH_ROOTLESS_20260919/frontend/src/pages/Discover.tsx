...
function compactStagingRecord(record: any) {
  return {
    entity: record?.entity ?? "",
    row: record?.row ?? null,
    target_write: record?.target_write === true,
  };
}
...
function compactDiscoveryForStorage(discovery: any) {
  ...
  const compactList = (value: unknown) =>
    Array.isArray(value) ? value.map(compactStagingRecord) : [];
  ...
  return {
    ...
    authoritative_promotion: discovery?.authoritative_promotion,
    summary: discovery?.summary ?? {},
    target_staging_plan: {
      ready_records: compactList(staging.ready_records),
      review_records: compactList(staging.review_records),
      quarantine_records: compactList(staging.quarantine_records),
      rejected_records: compactList(staging.rejected_records),
      referential_dependencies: Array.isArray(staging.referential_dependencies)
        ? staging.referential_dependencies.map((item: any) => ({
            entity: item?.entity ?? "",
            field: item?.field ?? "",
            row: item?.row ?? null,
            ...
            source: item?.source ?? "",
            target: item?.target ?? "",
            target_write: item?.target_write === true,
          }))
        : [],
      compact: true,
    },
...
function persistDiscoveryResult(discovery: any) {
  ...
  try {
    ...
    // STORAGE-QUOTA-001: remove only stale/derived discovery caches and retry.
    // The authoritative discovery remains server-side under migration_id.
...
function inferRelations(entities: EntityProfile[]) {
  ...
  for (const parent of entities) {
    const pk = `${singular(parent.stem)}_id`;
    if (!parent.columns.map((x) => x.toLowerCase()).includes(pk)) continue;
    for (const child of entities) {
      if (child.name === parent.name) continue;
      if (!child.columns.map((x) => x.toLowerCase()).includes(pk)) continue;
      const parentValues = new Set<string>();
      // orphanCount is populated later from previews when available; structural relation remains factual.
      out.push({ parent: parent.name, child: child.name, key: pk, orphanCount: 0 });
      void parentValues;
    }
  }
...
export default function Discover({
  ...
  const syncAuthoritativeEvidence = async () => {
    ...
    try {
      ...
      // A database source has no file path, so identify it by connection id
      // and let the backend read it through the read-only Source API.
      const sourceFilePath =
        result?.source?.filePath ||
        result?.source?.path ||
        syncPaths.sourcePath;
      ...
      const response = await apiPost<any>(
        "/api/v1/discovery/jobs",
        {
          ...
          // Saved UI-005 rules win; the path field is the fallback for a
          // rules document that was never pasted into the app.
          ...(Array.isArray(result?.requirement?.rules) &&
          result.requirement.rules.length
            ? { business_rules: result.requirement.rules }
            : syncPaths.businessRulesPath
              ? { business_rules_path: syncPaths.businessRulesPath }
              : { business_rules: [] }),
        }
      );
  ...
  const run = async () => {
    ...
    try {
      ...
      const sourceProfilesAll: EntityProfile[] = await Promise.all(sourceObjects.map(async (o) => {
        ...
        const matchedCount = serverScoped && typeof p.totalRows === "number" ? p.totalRows : scopedRows.length;
        const scannedCount = typeof p.scannedRows === "number" ? p.scannedRows : rows.length;
        return {
          schema: o.schema,
          name: o.name,
          stem: stem(o.name),
      ...
      const sourceProfiles = sourceProfilesAll;
      ...
      window.dispatchEvent(new CustomEvent("kmitora:unified-discovery-state", { detail: { ready: true, sourceEntities: sourceProfiles.length, targetEntities: targetProfiles.length, rules: requirement.rules.length } }));
      setResult(unified);
  ...
  return <div className="kud-page">
    ...
    <header className="kud-header">
      <div><div className="kud-eyebrow">STEP 04 · DISCOVER & UNDERSTAND</div><h1>Unified Enterprise Discovery</h1><p>Combine live source metadata, live target metadata and the saved business requirement into one governed migration understanding.</p></div>
      <div className="kud-env"><span>Environment</span><strong>DEV</strong><small>Production migration disabled</small></div>
    </header>
...
<section className="kud-safety"><strong>READ-ONLY DISCOVERY</strong><span>Source reads only · Target metadata/preview only · Production writes DISABLED · Cutover DISABLED</span></section>
    ...
    {ready && (
      <section style={{
        ...
        >
          {syncOpen ? "â–¾" : "â–¸"} Sync server-verified evidence (recommended before Request Approval)
        </button>
        ...
        {syncOpen && (
          <div style={{ marginTop: 10 }}>
            <p style={{ margin: "0 0 10px", color: "#64748b", fontSize: 13 }}>
              ...
              counts from the files below. Running this keeps Approval and Execute
              Migration in agreement â€” without it, Execute Migration may reject an
              approval whose numbers came only from this preview screen.
            </p>
            <div style={{ display: "grid", gap: 8, maxWidth: 640 }}>
              <label style={{ fontSize: 12, color: "#475569" }}>
                Source path (folder or file)
                <input
                  type="text"
                  value={syncPaths.sourcePath}
                  onChange={(e) => setSyncPaths((p) => ({ ...p, sourcePath: e.target.value }))}
                  style={{ width: "100%", padding: 6, marginTop: 2 }}
                  placeholder="Current connected source path"
                />
              </label>
              <label style={{ fontSize: 12, color: "#475569" }}>
                Target schema file
                <input
                  type="text"
                  value={syncPaths.targetPath}
                  onChange={(e) => setSyncPaths((p) => ({ ...p, targetPath: e.target.value }))}
                  style={{ width: "100%", padding: 6, marginTop: 2 }}
                  placeholder="Current connected target schema"
                />
              </label>
              <label style={{ fontSize: 12, color: "#475569" }}>
                Business rules file
                <input
                  type="text"
                  value={syncPaths.businessRulesPath}
                  onChange={(e) => setSyncPaths((p) => ({ ...p, businessRulesPath: e.target.value }))}
                  style={{ width: "100%", padding: 6, marginTop: 2 }}
                  placeholder="C:\...\BUSINESS_RULES\requirements.txt"
                />
              </label>
            </div>
            ...
            {syncSummary && !syncError && (
              <p style={{ color: "#166534", marginTop: 8, fontSize: 13 }}>
                Synced â€” Ready {syncSummary.staging_ready_count ?? 0} ·{" "}
                ...
                Go to Validate/Migrate â€” these numbers will now match what
                Execute Migration checks against.
              </p>
            )}
          </div>
    ...
    {result?.sourceScope?.mode === "FILTERED" && <section className="kud-safety"><strong>SCOPE ENFORCED</strong><span>{result.sourceScope.description} · Rows shown below are matched rows after deterministic requirement-derived filtering.</span></section>}
