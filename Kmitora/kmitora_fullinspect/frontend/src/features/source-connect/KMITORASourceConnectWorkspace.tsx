...
const ports: Record<Exclude<SourceType,"file">, number> = { postgresql:5432, oracle:1521, sqlserver:1433, mysql:3306, snowflake:443 };
...
export default function KMITORASourceConnectWorkspace({adapter}:{adapter:SourceConnectAdapter}) {
  ...
  const openSource=async(source:SourceConnection)=>{ const previousActiveSource=localStorage.getItem("kmitora.active.sourceId"); localStorage.setItem("kmitora.active.sourceId", source.id); if(previousActiveSource && previousActiveSource!==source.id){ localStorage.removeItem("kmitora.discovery.unified.v1"); localStorage.removeItem("kmitora.dev.discoveryResult"); localStorage.removeItem("kmitora.dev.migrationId"); } setSelectedSource(source); setSelectedObject(null); setPreview(null); setPreviewError("");
      ...
      <div className="ks-card">
        ...
        <div className="ks-form-grid">
          ...
          <label>Source Type<select value={draft.type} onChange={e=>updateType(e.target.value as SourceType)}>
            <option value="postgresql">PostgreSQL</option><option value="oracle">Oracle</option><option value="sqlserver">SQL Server</option><option value="mysql">MySQL</option><option value="snowflake">Snowflake</option><option value="file">File System</option>
          </select></label>
              ...
              >
                ...
                >
                  <div>
                    ...
                    <span>
                      {source.type} · {source.database || source.host || "file source"}
                    </span>
                  </div>
      ...
      <div className="ks-browser-grid">
        <aside className="ks-object-list">
          {objects.map(obj=><div key={`${obj.schema}.${obj.name}`} className={`ks-object-row ${selectedObject?.name===obj.name&&selectedObject?.schema===obj.schema?"active":""}`}>
            <button className="ks-object-main" onClick={()=>void openPreview(obj)}><strong title={obj.name}>{obj.name}</strong><span>{obj.schema} · {obj.type}</span></button>
