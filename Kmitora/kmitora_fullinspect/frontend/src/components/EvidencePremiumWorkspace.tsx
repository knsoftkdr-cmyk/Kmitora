...
export default function EvidencePremiumWorkspace() {
  ...
  const snapshot = useMemo(() => {
    ...
    const categories = [
      Boolean(evidence?.discovery_summary),
      ...
      transformationEvidence.length > 0 || recordResults.length > 0,
      Boolean(evidence?.safety),
    ];
    ...
    const targetWrites = numberOf(evidence?.safety?.target_write_count);
    const productionActions = numberOf(evidence?.safety?.production_action_count);
    ...
    const artifactCount =
      recordResults.length + transformationEvidence.length + businessRules.length;

    const decisionTraceCount =
      Object.keys(evidence?.approval_summary ?? {}).length +
      Object.keys(evidence?.execution_summary ?? {}).length +
      Object.keys(evidence?.reconciliation_summary ?? {}).length;
  ...
  return (
    <div className="evidencePremium" ref={rootRef}>
      <header className="evidenceHeader">
        <div>
          <span className="evidenceEyebrow">STEP 08 · AUDIT & EVIDENCE</span>
      ...
      <section className="evidenceKpis">
        ...
        <article>
          ...
          <small>Approval / execution / reconciliation summaries</small>
        </article>
        <article>
        ...
        <article>
          ...
          <strong>{snapshot.createdAt ? "AVAILABLE" : "—"}</strong>
      ...
      <nav className="evidenceTabs">
        {([
          ...
          ["TRACE", `Decision Trace (${snapshot.decisionTraceCount})`, GitBranch],
        ...
        <div className="evidenceBody">
          <div className="evidenceTopGrid">
            ...
            <section>
              ...
              <div className="evidenceHashState">
                ...
                <p>Target writes: {snapshot.targetWrites} · Production actions: {snapshot.productionActions}</p>
              </div>
          ...
          <section className="evidenceAuditScope">
            ...
            <div className="evidenceScopeCards">
              ...
              <article><ClipboardCheck size={16}/><span>Reconciliation Proof</span><strong>{snapshot.reconciliationPassed ? "PASS" : "—"}</strong><small>Authoritative reconciliation result</small><em>{snapshot.reconciliationPassed ? "AVAILABLE" : "AWAITING"}</em></article>
              <article><Database size={16}/><span>Export Safety</span><strong>{snapshot.targetWrites}</strong><small>Target production writes</small><em>READ ONLY</em></article>
          ...
          <div className="evidenceBottomGrid">
            <section>
              ...
              <div className="evidenceSummaryGrid">
                ...
                <div>
                  <div><span>Migration ID</span><strong>{snapshot.evidence?.migration_id ?? "—"}</strong></div>
                  <div><span>Approval ID</span><strong>{snapshot.evidence?.approval_id ?? "—"}</strong></div>
                  <div><span>Execution ID</span><strong>{snapshot.executionId}</strong></div>
                  <div><span>Reconciliation ID</span><strong>{snapshot.evidence?.reconciliation_id ?? "—"}</strong></div>
                </div>
              </div>
      ...
      {view === "INTELLIGENCE" && (
        <div className="evidenceIntelligence">
          ...
          <div className="evidenceIntelCards">
            ...
            <article><ShieldCheck size={18}/><span>Safety</span><strong>{snapshot.safetyPreserved ? "PRESERVED" : "REVIEW"}</strong><small>Mutation evidence only</small></article>
          </div>
