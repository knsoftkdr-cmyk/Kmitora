...
$Test = Invoke-KmitoraJson -Path '/v1/tests' -Method POST -Body @{ test_type = 'LIFECYCLE_RUNTIME_029D_QUALIFICATION'; read_only = $true }
Check-LifecycleRuntime -Response $Test -Stage 'TEST' -Label '/v1/tests' -ExpectedStatus 202

$Evidence = Invoke-KmitoraJson -Path '/v1/evidence' -Method POST -Body @{ purpose = 'LIFECYCLE_RUNTIME_029D_QUALIFICATION'; read_only = $true }
Check-LifecycleRuntime -Response $Evidence -Stage 'EVIDENCE' -Label '/v1/evidence' -ExpectedStatus 202
