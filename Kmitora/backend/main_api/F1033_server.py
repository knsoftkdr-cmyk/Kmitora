...
# PERSISTENT-UI-STATE-001: durable DEV lifecycle context survives service/browser restarts.
_UI_STATE_DIR = Path(__file__).resolve().parent / "runtime_state"
_UI_STATE_FILE = _UI_STATE_DIR / "ui_lifecycle_state.json"
_UI_STATE_LOCK = threading.RLock()

def _load_persistent_ui_state() -> dict[str, Any]:
    with _UI_STATE_LOCK:
        try:
            if not _UI_STATE_FILE.exists():
                return {}
            payload = json.loads(_UI_STATE_FILE.read_text(encoding="utf-8"))
            return payload if isinstance(payload, dict) else {}
        except Exception:
            return {}

def _save_persistent_ui_state(update: dict[str, Any]) -> dict[str, Any]:
    allowed = {
        "active_source_id", "active_target_id", "active_migration_id",
        "business_requirement", "last_good_discovery", "stage_outputs",
        "updated_at"
    }
    clean = {k: v for k, v in dict(update or {}).items() if k in allowed}
    # Explicitly reject obvious secret-bearing fields anywhere in top-level state.
    forbidden = {"password", "secret", "token", "api_key", "private_key", "credential"}
    for key in clean:
        if str(key).lower() in forbidden:
            raise ValueError("Secret-bearing UI state is not persistable")
    with _UI_STATE_LOCK:
        current = _load_persistent_ui_state()
        current.update(clean)
        current["updated_at"] = utc_now()
        _UI_STATE_DIR.mkdir(parents=True, exist_ok=True)
        tmp = _UI_STATE_FILE.with_suffix(".tmp")
        tmp.write_text(json.dumps(current, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(_UI_STATE_FILE)
        return current

# LIFECYCLE-CONTEXT-001: persist authoritative discoveries by migration id so
# every lifecycle stage can rehydrate the same governed context after service
# or browser restarts. Files contain migration evidence only; connection
# passwords are never written here.
_DISCOVERY_STATE_DIR = _UI_STATE_DIR / "discoveries"
_DISCOVERY_STATE_LOCK = threading.RLock()

def _discovery_state_path(migration_id: str) -> Path:
    safe = "".join(ch for ch in str(migration_id) if ch.isalnum() or ch in {"-", "_"})
    if not safe:
        raise ValueError("migration_id is required")
    return _DISCOVERY_STATE_DIR / f"{safe}.json"

def _persist_discovery_state(discovery: dict[str, Any]) -> None:
    migration_id = str(discovery.get("migration_id", "")).strip()
    if not migration_id:
        return
    with _DISCOVERY_STATE_LOCK:
        _DISCOVERY_STATE_DIR.mkdir(parents=True, exist_ok=True)
        path = _discovery_state_path(migration_id)
        tmp = path.with_suffix(".tmp")
        tmp.write_text(json.dumps(discovery, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(path)

def _load_discovery_state(migration_id: str) -> dict[str, Any] | None:
    try:
        path = _discovery_state_path(migration_id)
    except ValueError:
        return None
...
def _get_authoritative_discovery(migration_id: str) -> dict[str, Any] | None:
    with _lock:
        discovery = _runtime_state.get("discoveries", {}).get(migration_id)
    if isinstance(discovery, dict):
        return discovery
    discovery = _load_discovery_state(migration_id)
    if discovery is not None:
        with _lock:
            _runtime_state.setdefault("discoveries", {})[migration_id] = discovery
            _runtime_state["last_discovery_id"] = migration_id
    return discovery

# APPROVAL_CONTEXT_001: approvals are authoritative server evidence, not browser
# state. Persist them by id so an A000 restart cannot turn a valid approval into
# a stale localStorage-only ghost. No secrets or target credentials are stored.
_APPROVAL_STATE_DIR = _UI_STATE_DIR / "approvals"
_APPROVAL_STATE_LOCK = threading.RLock()

def _approval_state_path(approval_id: str) -> Path:
    safe = "".join(ch for ch in str(approval_id) if ch.isalnum() or ch in {"-", "_"})
    if not safe:
        raise ValueError("approval_id is required")
    return _APPROVAL_STATE_DIR / f"{safe}.json"

def _persist_approval_state(approval: dict[str, Any]) -> None:
    approval_id = str(approval.get("id", "")).strip()
    if not approval_id:
        return
    payload = dict(approval)
    payload["persistence_version"] = "APPROVAL_CONTEXT_001"
    with _APPROVAL_STATE_LOCK:
        _APPROVAL_STATE_DIR.mkdir(parents=True, exist_ok=True)
        path = _approval_state_path(approval_id)
        tmp = path.with_suffix(".tmp")
        tmp.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(path)

def _load_approval_state(approval_id: str) -> dict[str, Any] | None:
    try:
        path = _approval_state_path(approval_id)
    except ValueError:
        return None
...
def _get_authoritative_approval(approval_id: str) -> dict[str, Any] | None:
    approval_id = str(approval_id or "").strip()
    if not approval_id:
        return None
    with _lock:
        approval = _runtime_state.get("approvals", {}).get(approval_id)
    if isinstance(approval, dict):
        return approval
    approval = _load_approval_state(approval_id)
    if approval is not None:
        with _lock:
            _runtime_state.setdefault("approvals", {})[approval_id] = approval
    return approval

def _list_authoritative_approvals(migration_id: str = "") -> list[dict[str, Any]]:
    combined: dict[str, dict[str, Any]] = {}
    with _lock:
        for approval_id, approval in _runtime_state.get("approvals", {}).items():
            if isinstance(approval, dict):
                combined[str(approval_id)] = dict(approval)
    with _APPROVAL_STATE_LOCK:
        try:
            if _APPROVAL_STATE_DIR.exists():
                for path in _APPROVAL_STATE_DIR.glob("*.json"):
                    try:
                        approval = json.loads(path.read_text(encoding="utf-8"))
                    except Exception:
                        continue
                    if isinstance(approval, dict) and str(approval.get("id", "")).strip():
                        combined[str(approval["id"])] = approval
        except Exception:
            pass
    ...
    if migration_id:
        items = [
            ...
            if str((item.get("input") or {}).get("migration_id") or item.get("migration_id") or "").strip() == migration_id
            and item.get("superseded") is not True
        ]
    items.sort(key=lambda item: str(item.get("requested_at", "")), reverse=True)
    return items

# EXECUTION_CONTEXT_001: execution evidence is authoritative server state and
# must survive A000 restarts. Persist DEV dry-run and post-load execution
# certificates by execution id; browser storage is display-only.
_EXECUTION_STATE_DIR = _UI_STATE_DIR / "executions"
_EXECUTION_STATE_LOCK = threading.RLock()

def _execution_state_path(execution_id: str) -> Path:
    safe = "".join(ch for ch in str(execution_id) if ch.isalnum() or ch in {"-", "_"})
    if not safe:
        raise ValueError("execution_id is required")
    return _EXECUTION_STATE_DIR / f"{safe}.json"

def _persist_execution_state(execution: dict[str, Any]) -> None:
    execution_id = str(execution.get("execution_id", "")).strip()
    if not execution_id:
        return
    payload = dict(execution)
    payload["persistence_version"] = "EXECUTION_CONTEXT_001"
    with _EXECUTION_STATE_LOCK:
        _EXECUTION_STATE_DIR.mkdir(parents=True, exist_ok=True)
        path = _execution_state_path(execution_id)
        tmp = path.with_suffix(".tmp")
        tmp.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(path)

def _load_execution_state(execution_id: str) -> dict[str, Any] | None:
    try:
        path = _execution_state_path(execution_id)
    except ValueError:
        return None
...
def _get_authoritative_execution(execution_id: str) -> dict[str, Any] | None:
    execution_id = str(execution_id or "").strip()
    if not execution_id:
        return None
    with _lock:
        execution = _runtime_state.get("executions", {}).get(execution_id)
    if isinstance(execution, dict):
        return execution
    execution = _load_execution_state(execution_id)
    if execution is not None:
        with _lock:
            _runtime_state.setdefault("executions", {})[execution_id] = execution
            _runtime_state["last_execution_id"] = execution_id
    return execution

# RECONCILIATION_EVIDENCE_STORAGE_001: reconciliation and evidence artifacts
# are authoritative server-side runtime records. Browser storage keeps only
# compact certificates/identifiers and must never hold full evidence payloads.
_RECONCILIATION_STATE_DIR = _UI_STATE_DIR / "reconciliations"
_RECONCILIATION_STATE_LOCK = threading.RLock()
_EVIDENCE_STATE_DIR = _UI_STATE_DIR / "evidence"
_EVIDENCE_STATE_LOCK = threading.RLock()

def _safe_runtime_id(value: str, field: str) -> str:
    safe = "".join(ch for ch in str(value) if ch.isalnum() or ch in {"-", "_"})
    if not safe:
        raise ValueError(f"{field} is required")
    return safe

def _reconciliation_state_path(reconciliation_id: str) -> Path:
    return _RECONCILIATION_STATE_DIR / f"{_safe_runtime_id(reconciliation_id, 'reconciliation_id')}.json"

def _evidence_state_path(evidence_id: str) -> Path:
    return _EVIDENCE_STATE_DIR / f"{_safe_runtime_id(evidence_id, 'evidence_id')}.json"

def _persist_reconciliation_state(reconciliation: dict[str, Any]) -> None:
    reconciliation_id = str(reconciliation.get("reconciliation_id", "")).strip()
    if not reconciliation_id:
        return
    payload = dict(reconciliation)
    payload["persistence_version"] = "RECONCILIATION_EVIDENCE_STORAGE_001"
    with _RECONCILIATION_STATE_LOCK:
        _RECONCILIATION_STATE_DIR.mkdir(parents=True, exist_ok=True)
        path = _reconciliation_state_path(reconciliation_id)
        tmp = path.with_suffix(".tmp")
        tmp.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(path)

def _load_reconciliation_state(reconciliation_id: str) -> dict[str, Any] | None:
    try:
        path = _reconciliation_state_path(reconciliation_id)
    except ValueError:
        return None
...
def _get_authoritative_reconciliation(reconciliation_id: str) -> dict[str, Any] | None:
    reconciliation_id = str(reconciliation_id or "").strip()
    if not reconciliation_id:
        return None
    with _lock:
        item = _runtime_state.get("reconciliations", {}).get(reconciliation_id)
    if isinstance(item, dict):
        return item
    item = _load_reconciliation_state(reconciliation_id)
    if item is not None:
        with _lock:
            _runtime_state.setdefault("reconciliations", {})[reconciliation_id] = item
            _runtime_state["last_reconciliation_id"] = reconciliation_id
    return item

def _persist_evidence_state(evidence: dict[str, Any]) -> None:
    evidence_id = str(evidence.get("evidence_id", "")).strip()
    if not evidence_id:
        return
    payload = dict(evidence)
    payload["persistence_version"] = "RECONCILIATION_EVIDENCE_STORAGE_001"
    with _EVIDENCE_STATE_LOCK:
        _EVIDENCE_STATE_DIR.mkdir(parents=True, exist_ok=True)
        path = _evidence_state_path(evidence_id)
        tmp = path.with_suffix(".tmp")
        tmp.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        tmp.replace(path)

def _load_evidence_state(evidence_id: str) -> dict[str, Any] | None:
    try:
        path = _evidence_state_path(evidence_id)
    except ValueError:
        return None
...
def _get_authoritative_evidence(evidence_id: str) -> dict[str, Any] | None:
    evidence_id = str(evidence_id or "").strip()
    if not evidence_id:
        return None
    with _lock:
        item = _runtime_state.get("evidence_packages", {}).get(evidence_id)
    if isinstance(item, dict):
        return item
    item = _load_evidence_state(evidence_id)
    if item is not None:
        with _lock:
            _runtime_state.setdefault("evidence_packages", {})[evidence_id] = item
            _runtime_state["last_evidence_id"] = evidence_id
    return item

def _list_authoritative_executions(migration_id: str = "") -> list[dict[str, Any]]:
    combined: dict[str, dict[str, Any]] = {}
    with _lock:
        for execution_id, execution in _runtime_state.get("executions", {}).items():
            if isinstance(execution, dict):
                combined[str(execution_id)] = dict(execution)
    with _EXECUTION_STATE_LOCK:
        try:
            if _EXECUTION_STATE_DIR.exists():
                for path in _EXECUTION_STATE_DIR.glob("*.json"):
                    try:
                        execution = json.loads(path.read_text(encoding="utf-8"))
                    except Exception:
                        continue
                    if isinstance(execution, dict) and str(execution.get("execution_id", "")).strip():
                        combined[str(execution["execution_id"])] = execution
        except Exception:
            pass
    items = list(combined.values())
    if migration_id:
        items = [item for item in items if str(item.get("migration_id", "")).strip() == migration_id]
    items.sort(key=lambda item: str(item.get("completed_at") or item.get("started_at") or ""), reverse=True)
    return items

def _supersede_existing_approvals(migration_id: str, keep_id: str = "") -> None:
    for approval in _list_authoritative_approvals(migration_id):
        approval_id = str(approval.get("id", "")).strip()
        if not approval_id or approval_id == keep_id:
            continue
        approval["superseded"] = True
        approval["superseded_at"] = utc_now()
        with _lock:
            _runtime_state.setdefault("approvals", {})[approval_id] = dict(approval)
        _persist_approval_state(approval)

# LIFECYCLE-CONTEXT-002: recover the most recent persisted discovery when the
# active migration id was lost during a service/browser restart. This is DEV
# lifecycle recovery only and never enables production writes.
def _recover_latest_discovery() -> dict[str, Any] | None:
    with _lock:
        last_id = str(_runtime_state.get("last_discovery_id") or "").strip()
        if last_id:
            item = _runtime_state.get("discoveries", {}).get(last_id)
            if isinstance(item, dict):
                return item
    with _DISCOVERY_STATE_LOCK:
        try:
            if not _DISCOVERY_STATE_DIR.exists():
                return None
            candidates = sorted(_DISCOVERY_STATE_DIR.glob("*.json"), key=lambda x: x.stat().st_mtime, reverse=True)
            for path in candidates:
                try:
                    payload = json.loads(path.read_text(encoding="utf-8"))
                except Exception:
                    continue
                if isinstance(payload, dict) and str(payload.get("migration_id") or "").strip():
                    migration_id = str(payload.get("migration_id")).strip()
                    with _lock:
                        _runtime_state.setdefault("discoveries", {})[migration_id] = payload
                        _runtime_state["last_discovery_id"] = migration_id
                    try:
                        _save_persistent_ui_state({"active_migration_id": migration_id})
                    except Exception:
                        pass
                    return payload
        except Exception:
...
def _compact_lifecycle_context(discovery: dict[str, Any] | None, state: dict[str, Any]) -> dict[str, Any]:
    discovery = discovery or {}
    summary = discovery.get("summary", {}) if isinstance(discovery.get("summary"), dict) else {}
    staging = discovery.get("target_staging_plan", {}) if isinstance(discovery.get("target_staging_plan"), dict) else {}
    source = discovery.get("source", {}) if isinstance(discovery.get("source"), dict) else {}
    target = discovery.get("target", {}) if isinstance(discovery.get("target"), dict) else {}
    relationships = discovery.get("relationships", []) if isinstance(discovery.get("relationships"), list) else []
    findings_raw = discovery.get("quality_findings", []) if isinstance(discovery.get("quality_findings"), list) else []
    findings = [item for item in findings_raw if isinstance(item, dict)]
    rules = discovery.get("business_rules", []) if isinstance(discovery.get("business_rules"), list) else []
    return {
        "migration_id": str(discovery.get("migration_id") or state.get("active_migration_id") or ""),
        "status": str(discovery.get("status") or ""),
        "source_entity_count": int(summary.get("source_entity_count", len(source.get("entities", []) if isinstance(source.get("entities"), list) else [])) or 0),
        "target_entity_count": int(summary.get("target_entity_count", len(target.get("entities", []) if isinstance(target.get("entities"), list) else [])) or 0),
        "business_rule_count": int(summary.get("business_rule_count", len(rules)) or 0),
        "relationship_count": int(summary.get("relationship_count", len(relationships)) or 0),
        "rows_observed": int(
            summary.get("rows_observed",
                summary.get("source_rows_observed",
                    summary.get("source_rows_matched",
                        summary.get("source_rows_scanned",
                            sum(
                                int((entity or {}).get("rows_matched", (entity or {}).get("rows_scanned", 0)) or 0)
        ...
        "quality_finding_count": int(summary.get("quality_finding_count", len(findings)) or 0),
        "staging_ready_count": int(summary.get("staging_ready_count", staging.get("ready_count", len(staging.get("ready_records", []) if isinstance(staging.get("ready_records"), list) else []))) or 0),
        "staging_review_count": int(summary.get("staging_review_count", staging.get("review_count", len(staging.get("review_records", []) if isinstance(staging.get("review_records"), list) else []))) or 0),
        "staging_quarantine_count": int(summary.get("staging_quarantine_count", staging.get("quarantine_count", len(staging.get("quarantine_records", []) if isinstance(staging.get("quarantine_records"), list) else []))) or 0),
        "staging_rejected_count": int(summary.get("staging_rejected_count", staging.get("rejected_count", len(staging.get("rejected_records", []) if isinstance(staging.get("rejected_records"), list) else []))) or 0),
        # LIFECYCLE-CONTEXT-004: never expose the live mutable stage_outputs
        # mapping inside the stage context. A direct reference creates a cycle
        # when the current stage result is inserted back into stage_outputs
        # (result -> context -> stage_outputs -> result), causing json.dumps to
        # fail with "Circular reference detected". Keep only an immutable,
        # compact snapshot of prior stage status/progress for context display.
        "stage_outputs": {
            str(name): {
            ...
            for name, value in (state.get("stage_outputs", {}) if isinstance(state.get("stage_outputs"), dict) else {}).items()
            if isinstance(value, dict)
        },
    }

def _evaluate_lifecycle_stage(stage: str, discovery: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    stage = str(stage).strip().lower()
    allowed = ["detect", "diagnose", "predict", "recommend", "simulate"]
    if stage not in allowed:
        raise ValueError("Unsupported lifecycle stage")
    context = _compact_lifecycle_context(discovery, state)
    findings_raw = discovery.get("quality_findings", []) if isinstance(discovery.get("quality_findings"), list) else []
    findings = [item for item in findings_raw if isinstance(item, dict)]
    transformations = discovery.get("transformation_plan", []) if isinstance(discovery.get("transformation_plan"), list) else []
    staging = discovery.get("target_staging_plan", {}) if isinstance(discovery.get("target_staging_plan"), dict) else {}
    outputs = state.get("stage_outputs", {}) if isinstance(state.get("stage_outputs"), dict) else {}
    predecessors = {"diagnose":"detect", "predict":"diagnose", "recommend":"predict", "simulate":"recommend"}
    predecessor = predecessors.get(stage)
    if predecessor and str((outputs.get(predecessor) or {}).get("status", "")) != "DONE":
        raise ValueError(f"{predecessor.title()} must complete before {stage.title()}")
    result: dict[str, Any] = {
        "stage": stage, "status": "DONE", "progress": 100,
        "migration_id": context["migration_id"], "evaluated_at": utc_now(),
        "context": context, "source_write_executed": False,
        "target_write_executed": False, "production_action_executed": False,
    }
    if stage == "detect":
        ...
        result["metrics"] = {"findings": len(findings), "severity_counts": severities}
        result["message"] = f"Evaluated {context['rows_observed']} discovered records against {context['business_rule_count']} rules; {len(findings)} evidence-backed findings recorded."
    elif stage == "diagnose":
        buckets: dict[str, int] = {}
        for item in findings:
            category = str(item.get("category") or item.get("type") or item.get("code") or "DATA_QUALITY").upper()
            buckets[category] = buckets.get(category, 0) + 1
        result["metrics"] = {"diagnosed_findings": len(findings), "root_cause_buckets": buckets}
        result["message"] = f"Diagnosed {len(findings)} findings using rules, mappings and dependency evidence."
    elif stage == "predict":
        at_risk = context["staging_review_count"] + context["staging_quarantine_count"] + context["staging_rejected_count"]
        result["metrics"] = {"records_at_risk": at_risk, "ready_records": context["staging_ready_count"], "relationship_count": context["relationship_count"]}
        result["message"] = f"Projected downstream risk for {at_risk} non-ready records across {context['relationship_count']} dependencies."
    elif stage == "recommend":
        result["metrics"] = {"transformation_candidates": len(transformations), "review_candidates": context["staging_review_count"] + context["staging_quarantine_count"]}
        result["message"] = f"Prepared {len(transformations)} governed transformation/remediation candidates from the authoritative discovery plan."
    else:
        total = context["staging_ready_count"] + context["staging_review_count"] + context["staging_quarantine_count"] + context["staging_rejected_count"]
        result["metrics"] = {
            "simulated_records": total, "ready": context["staging_ready_count"],
            "review": context["staging_review_count"], "quarantine": context["staging_quarantine_count"],
            "rejected": context["staging_rejected_count"], "source_writes": 0, "target_writes": 0,
        }
        result["message"] = f"DEV-safe simulation classified {total} records without source or target writes."
    return result
...
def _read_connected_source_entities(
    ...
    REPEAT-DISCOVERY-001: file-backed registered sources are resolved from
    their configured root path and read through the same recursive
    multi-format reader used by direct source_path discovery. This avoids
    preview limits, CSV-only assumptions, and first-run/second-run drift.

    Database sources continue to use the Source API's read-only preview
    endpoint.
    """
    quoted = urllib.parse.quote(source_id)

    # Resolve the registered source itself first. Public source metadata does
    # not expose credentials but does include the file root for file sources.
    try:
        registered_sources = _source_api_get("/v1/sources")
    except Exception:
        registered_sources = []

    if isinstance(registered_sources, list):
        record = next(
            (item for item in registered_sources
             if isinstance(item, dict) and str(item.get("id")) == source_id),
            None,
        )
        if record and str(record.get("type", "")).lower() == "file":
            file_path_raw = str(record.get("filePath") or "").strip()
            if not file_path_raw:
                raise ValueError(
                    f"Connected file source {source_id} has no configured filePath"
                )
            file_path = Path(file_path_raw).resolve()
            if not file_path.exists():
                raise ValueError(
                    f"Connected file source path does not exist: {file_path}"
                )
            return _read_recursive_file_source(file_path)
...
def _infer_relationships(
    ...
    # RELATIONSHIP-DAG-001: infer business keys from schema semantics and
    # actual uniqueness rather than assuming the first column is a primary
    # key. This prevents child entities such as address_history from
    # misclassifying customer_id as their own PK when address_id is the
    # unique row key.
    def _key_candidates(entity_name: str) -> list[str]:
        name = entity_name.lower().strip()
        singular = name[:-1] if name.endswith("s") else name
        semantic_bases = [singular, name]
        generic_suffixes = (
            "_master", "_history", "_detail", "_details", "_header",
            "_transaction", "_transactions", "_table", "_data",
        )
        for suffix in generic_suffixes:
            if singular.endswith(suffix):
                base = singular[: -len(suffix)].strip("_")
                if base:
                    semantic_bases.append(base)
        parts = [p for p in singular.split("_") if p]
        if parts:
            semantic_bases.extend(parts)
        dedup_bases = []
        for base in semantic_bases:
            if base and base not in dedup_bases:
                dedup_bases.append(base)
        suffixes = ("_id", "_key", "_code", "_no", "_number")
        return [f"{base}{suffix}" for base in dedup_bases for suffix in suffixes] + ["id"]

    def _nonblank_values(entity: dict[str, Any], field: str) -> list[str]:
        return [
            str(row.get(field, "") or "").strip()
            for row in entity.get("_rows", [])
            if str(row.get(field, "") or "").strip()
        ]

    def _is_unique(entity: dict[str, Any], field: str) -> bool:
        values = _nonblank_values(entity, field)
    ...
    for entity in source_entities:
        ...
        candidates = [fields[c] for c in _key_candidates(name) if c in fields]

        # Prefer an id-like candidate that is actually unique in the
        # authoritative rows. Fall back to the first semantic candidate only
        # when rows are unavailable (metadata-only discovery). Never assume
        # the first physical column is the entity's primary key.
        unique = next((field for field in candidates if _is_unique(entity, field)), None)
        if unique:
            parent_keys[name] = unique
        elif candidates and not entity.get("_rows"):
            parent_keys[name] = candidates[0]
...
def _suggest_mappings(
    ...
    def _resolve_target_entity(
        source_entity: str,
        source_fields: list[Any] | None = None,
    ) -> str | None:
        """Resolve a source entity to one authoritative target table.

        TARGET_MAPPING_001: execution must never assume that the target table
        has the same name as the source entity.  Exact/common-prefix matches
        are preferred; otherwise a unique semantic suffix candidate is chosen
        only when its field overlap makes the choice deterministic.
        """
        singular = (
            source_entity[:-1]
        ...
        for target_name, target_fields in target_lookup.items():
            target_norm = _norm(target_name)
            semantic_suffix = (
                target_norm.endswith(source_norm)
                or source_norm.endswith(target_norm)
            )
            if not semantic_suffix:
                continue

            target_field_norms = {_norm(name) for name in target_fields}
            overlap = len(source_field_norms & target_field_norms)
            candidates.append((overlap, len(target_norm), target_name))

        if not candidates:
            return None

        # Highest field overlap wins.  A tie is deliberately unresolved so a
        # domain with multiple similarly named targets cannot be guessed.
        ...
        # If there is exactly one semantic candidate, it is deterministic even
        # when a schema exposes no column metadata during discovery.
        if len(candidates) == 1:
...
def run_discovery(data: dict[str, Any]) -> dict[str, Any]:
    ...
    supporting_artifacts = data.get("supporting_artifacts") or []
    ...
    # LIFECYCLE-CONTEXT-001: persist the authoritative discovery and make its
    # migration id the single lifecycle correlation key used by all stages.
    _persist_discovery_state(result)
    _save_persistent_ui_state({
        "active_migration_id": result["migration_id"],
        "stage_outputs": {},
    })
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code: int, obj: dict[str, Any]) -> None:
        # BEGIN KMITORA_A000_LIFECYCLE_UNIFIED_RUNTIME_029
        try:
            import os as _kmitora_os_029
            import sys as _kmitora_sys_029
            _kmitora_backend_root_029 = _kmitora_os_029.path.dirname(
                _kmitora_os_029.path.dirname(_kmitora_os_029.path.abspath(__file__))
            )
            if _kmitora_backend_root_029 not in _kmitora_sys_029.path:
                _kmitora_sys_029.path.insert(0, _kmitora_backend_root_029)
            from a000_core.lifecycle_bridge import (
                project_lifecycle_response as _kmitora_project_lifecycle_029,
            )
            obj = _kmitora_project_lifecycle_029(
                path=getattr(self, "path", ""),
                code=code,
                envelope_obj=obj,
            )
        except Exception:
            # Projection is additive only. Existing lifecycle response always wins.
            pass
        # END KMITORA_A000_LIFECYCLE_UNIFIED_RUNTIME_029
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/v1/ui-state":
            return self._send(
                ...
                envelope(
                    "ui_persistent_state",
                    {
                        "state": _load_persistent_ui_state(),
                        "persistent": True,
                        "environment": "DEV",
                        "production_authorized": False,
                    },
        ...
        if path == "/v1/lifecycle-context":
            state = _load_persistent_ui_state()
            migration_id = str(state.get("active_migration_id") or "").strip()
            discovery = _get_authoritative_discovery(migration_id) if migration_id else None
            if discovery is None:
                discovery = _recover_latest_discovery()
                if discovery is not None:
                    migration_id = str(discovery.get("migration_id") or "").strip()
                    state = _load_persistent_ui_state()
            return self._send(
                200,
                envelope(
        ...
        if path.startswith("/v1/executions/"):
            execution_id = urllib.parse.unquote(path.split("/v1/executions/", 1)[1].strip())
            if not execution_id:
                return self._send(400, envelope("execution_error", {"message": "execution_id is required"}))
            execution = _get_authoritative_execution(execution_id)
            if execution is None:
                return self._send(404, envelope("execution_error", {"message": "Execution not found", "execution_id": execution_id}))
            return self._send(200, envelope("execution", execution))

        if path == "/v1/executions":
            parsed = urllib.parse.urlparse(self.path)
            params = urllib.parse.parse_qs(parsed.query)
            migration_id = str((params.get("migration_id") or [""])[0]).strip()
            items = _list_authoritative_executions(migration_id)
            return self._send(200, envelope("executions", {
                "items": items,
                "count": len(items),
        ...
        if path.startswith("/v1/evidence/"):
            evidence_id = urllib.parse.unquote(path.split("/v1/evidence/", 1)[1].strip())
            if not evidence_id:
                return self._send(400, envelope("evidence_error", {"message": "evidence_id is required"}))
            evidence = _get_authoritative_evidence(evidence_id)
            if evidence is None:
                return self._send(404, envelope("evidence_error", {"message": "Evidence package not found", "evidence_id": evidence_id}))
            return self._send(200, envelope("evidence", evidence))

        if path.startswith("/v1/reconciliations/"):
            reconciliation_id = urllib.parse.unquote(path.split("/v1/reconciliations/", 1)[1].strip())
            if not reconciliation_id:
                return self._send(400, envelope("reconciliation_error", {"message": "reconciliation_id is required"}))
            reconciliation = _get_authoritative_reconciliation(reconciliation_id)
            if reconciliation is None:
                return self._send(404, envelope("reconciliation_error", {"message": "Reconciliation not found", "reconciliation_id": reconciliation_id}))
            return self._send(200, envelope("reconciliation", reconciliation))
        ...
        if path.startswith("/v1/a000/discoveries/"):
            ...
            discovery = _get_authoritative_discovery(migration_id)
            if discovery is None:
                return self._send(
                    404,
                    envelope(
        ...
        if path == "/v1/approvals":
            query_values = urllib.parse.parse_qs(urllib.parse.urlsplit(self.path).query)
            migration_id = str((query_values.get("migration_id") or [""])[0]).strip()
            approvals = _list_authoritative_approvals(migration_id)
            return self._send(
                200,
                envelope(
                    "approvals",
        ...
        approval_get_prefix = "/v1/approvals/"
        if path.startswith(approval_get_prefix):
            approval_id = path[len(approval_get_prefix):].strip()
            approval = _get_authoritative_approval(approval_id)
            if approval is None or approval.get("superseded") is True:
                return self._send(404, envelope("approval_lookup_error", {
                    "message": "Approval request not found",
                    "approval_id": approval_id,
                    "authoritative": True,
                }))
            return self._send(200, envelope("approval", {
                **dict(approval),
                "authoritative": True,
                "persistent": True,
            }))
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/v1/ui-state":
            try:
                saved = _save_persistent_ui_state(data)
            except Exception as exc:
                return self._send(400, envelope("ui_persistent_state_error", {"message": str(exc)}))
            return self._send(200, envelope("ui_persistent_state_saved", {"state": saved, "persistent": True, "environment": "DEV", "production_authorized": False}))

        if path.startswith("/v1/lifecycle/stages/") or path == "/v1/lifecycle-stage":
            stage = (
                path.split("/v1/lifecycle/stages/", 1)[1].strip().lower()
                if path.startswith("/v1/lifecycle/stages/")
                else str(data.get("stage") or "").strip().lower()
            )
            state = _load_persistent_ui_state()
            migration_id = str(data.get("migration_id") or state.get("active_migration_id") or "").strip()
            discovery = _get_authoritative_discovery(migration_id) if migration_id else None
            if discovery is None:
                discovery = _recover_latest_discovery()
                if discovery is not None:
                    ...
                    state = _load_persistent_ui_state()
            if not migration_id:
                return self._send(400, envelope("lifecycle_stage_error", {"message": "No active migration_id. Run Unified Discovery first."}))
            if discovery is None:
                return self._send(404, envelope("lifecycle_stage_error", {"message": "Authoritative discovery not found", "migration_id": migration_id}))
            try:
                result = _evaluate_lifecycle_stage(stage, discovery, state)
                outputs = dict(state.get("stage_outputs", {}) if isinstance(state.get("stage_outputs"), dict) else {})
                outputs[stage] = result
                saved = _save_persistent_ui_state({"active_migration_id": migration_id, "stage_outputs": outputs})
                # Return a detached summary only. The authoritative full stage outputs
                # remain persisted server-side and are retrieved through lifecycle context.
                result["stage_outputs"] = {
                    ...
                    for name, value in (saved.get("stage_outputs", {}) if isinstance(saved.get("stage_outputs"), dict) else {}).items()
                    if isinstance(value, dict)
                }
                return self._send(200, envelope("lifecycle_stage_result", result))
            except ValueError as exc:
                return self._send(409, envelope("lifecycle_stage_error", {"message": str(exc), "migration_id": migration_id, "stage": stage}))
            except Exception as exc:
                return self._send(500, envelope("lifecycle_stage_error", {
                    "message": f"Lifecycle stage evaluation failed: {type(exc).__name__}: {exc}",
                    "migration_id": migration_id,
                    "stage": stage,
                }))
        ...
        if path == "/v1/a000/messages":
            ...
            assistant_ui_state = _load_persistent_ui_state()
            assistant_migration_id = str(
                data.get("migration_id")
                or assistant_ui_state.get("active_migration_id")
                or ""
            ).strip()
            assistant_discovery = (
                _get_authoritative_discovery(assistant_migration_id)
                if assistant_migration_id
                else _recover_latest_discovery()
            )
            base_reply = build_assistant_response(
                message,
                stage=str(data.get("stage", "")),
                ui_state=assistant_ui_state,
                discovery=assistant_discovery,
            ...
            # BEGIN KMITORA_A000_LIVE_UNIFIED_RUNTIME_027
            # 028A: make backend sibling package imports reliable when F1033_server.py
            # is launched directly by absolute path from any working directory.
            try:
                import os as _kmitora_os_028a
                import sys as _kmitora_sys_028a
                _kmitora_backend_root_028a = _kmitora_os_028a.path.dirname(
                    _kmitora_os_028a.path.dirname(_kmitora_os_028a.path.abspath(__file__))
                )
                if _kmitora_backend_root_028a not in _kmitora_sys_028a.path:
                    _kmitora_sys_028a.path.insert(0, _kmitora_backend_root_028a)
            
                from a000_core.live_bridge import (
                    run_live_unified_runtime as _kmitora_run_live_027,
                    fail_closed_live_runtime as _kmitora_fail_closed_027,
                )
                _kmitora_live_027 = _kmitora_run_live_027(
                    message=message,
                    request=data,
                    current_reply=reply,
                    learning_context=(
                        learning_context
                        if "learning_context" in locals()
                        else None
                    ),
                )
            except Exception as _kmitora_live_027_error:
                try:
                    # If the bridge module imported successfully but execution failed,
                    # use its governed fail-closed response.
                    _kmitora_live_027 = _kmitora_fail_closed_027(
                        message, _kmitora_live_027_error
                    )
                except Exception as _kmitora_live_027_fallback_error:
                    # Last-resort response stays schema-compatible so live qualification
                    # reports the real error instead of failing on a missing property.
                    _kmitora_live_027 = {
                        "patch_id": "A000_LIVE_UNIFIED_RUNTIME_027",
                        "runtime_hardening_patch": "A000_LIVE_RUNTIME_IMPORT_028A",
                        "status": "REVIEW",
                        "decision": "ABSTAIN_REVIEW",
                        "mode": "PLAN_ONLY",
                        "normal_message_path_integrated": True,
                        "error": str(_kmitora_live_027_error),
                        "fallback_error": str(_kmitora_live_027_fallback_error),
                        "safety": {
            ...
            if isinstance(reply, dict):
                reply["unified_runtime"] = _kmitora_live_027
            # END KMITORA_A000_LIVE_UNIFIED_RUNTIME_027
            return self._send(
                200,
                envelope(
                    "a000_message",
        ...
        allowed = {
            ...
            "/v1/post-load-executions": "post_load_execution",
        ...
        if path == "/v1/post-load-executions":
            dry_run_execution_id = str(data.get("dry_run_execution_id", "")).strip()
            migration_id = str(data.get("migration_id", "")).strip()
            approval_id = str(data.get("approval_id", "")).strip()
            target_id = str(data.get("target_id", "")).strip()
            environment = str(data.get("environment", "DEV")).strip().upper()
            write_results = data.get("write_results")
            target_counts = data.get("target_counts")
            total_loaded = int(data.get("total_loaded", 0) or 0)
            review_held = int(data.get("review_records_held", 0) or 0)
            quarantine_held = int(data.get("quarantine_records_held", 0) or 0)
            rejected_held = int(data.get("rejected_records_held", 0) or 0)

            if not all((dry_run_execution_id, migration_id, approval_id, target_id)):
                return self._send(400, envelope("post_load_error", {"message": "dry_run_execution_id, migration_id, approval_id and target_id are required"}))
            if environment != "DEV":
                return self._send(403, envelope("post_load_blocked", {"message": "Post-load execution registration is DEV-only"}))
            if not isinstance(write_results, list) or not isinstance(target_counts, dict):
                return self._send(400, envelope("post_load_error", {"message": "write_results array and target_counts object are required"}))

            # EXECUTION_CONTEXT_001: rehydrate every prerequisite from its
            # authoritative durable store. A000 restarts must never invalidate
            # a valid DEV dry-run/discovery/approval chain or accept browser-only evidence.
            dry_run = _get_authoritative_execution(dry_run_execution_id)
            discovery = _get_authoritative_discovery(migration_id)
            approval = _get_authoritative_approval(approval_id)

            if dry_run is None or discovery is None or approval is None:
                missing = []
                if dry_run is None:
                ...
                return self._send(409, envelope("post_load_blocked", {
                    "message": "Authoritative dry-run, discovery and approval evidence are required",
                    "missing_evidence": missing,
                }))
            if str(dry_run.get("status", "")).upper() != "DRY_RUN_COMPLETED":
                return self._send(409, envelope("post_load_blocked", {"message": "Parent execution must be DRY_RUN_COMPLETED"}))
            if str(dry_run.get("migration_id", "")).strip() != migration_id or str(dry_run.get("approval_id", "")).strip() != approval_id:
                return self._send(409, envelope("post_load_blocked", {"message": "Dry-run identity does not match migration/approval"}))
            if str(approval.get("status", "")).upper() != "APPROVED" or approval.get("approved") is not True:
                return self._send(403, envelope("post_load_blocked", {"message": "Approval is no longer authoritative"}))

            staging = discovery.get("target_staging_plan", {}) if isinstance(discovery.get("target_staging_plan", {}), dict) else {}
            ready_records = staging.get("ready_records", []) if isinstance(staging.get("ready_records", []), list) else []
            expected_ready = len(ready_records)
            inserted = sum(int(item.get("inserted", 0) or 0) for item in write_results if isinstance(item, dict))
            attempted = sum(int(item.get("attempted", 0) or 0) for item in write_results if isinstance(item, dict))
            counted = sum(int(v or 0) for v in target_counts.values())

            if total_loaded != inserted or total_loaded != counted or inserted != attempted or total_loaded != expected_ready:
                return self._send(409, envelope("post_load_blocked", {
                    "message": "DEV replace-load counts do not reconcile with authoritative ready population",
                    "expected_ready": expected_ready, "attempted": attempted, "inserted": inserted, "target_count_total": counted, "total_loaded": total_loaded
                }))

            post_load_execution_id = str(uuid.uuid4())
            post_load = {
                "execution_id": post_load_execution_id,
                "parent_dry_run_execution_id": dry_run_execution_id,
                "migration_id": migration_id,
                "approval_id": approval_id,
                "target_id": target_id,
                "status": "POST_LOAD_COMPLETED",
                "execution_gate": "PASS",
                "execution_mode": "DEV_REPLACE_LOAD",
                "environment": "DEV",
                "input_record_count": expected_ready,
                "simulated_record_count": int(dry_run.get("simulated_record_count", expected_ready) or expected_ready),
                "success_count": expected_ready,
                "failure_count": 0,
                "loaded_record_count": total_loaded,
                "record_results": dry_run.get("record_results", []),
                "write_results": write_results,
                "target_counts": target_counts,
                "disposition_summary": {"ready": expected_ready, "review": review_held, "quarantine": quarantine_held, "rejected": rejected_held},
                "held_records_preserved": True,
                "review_records_written": False,
                "quarantine_records_written": False,
                "rejected_records_written": False,
                "source_write_executed": False,
            ...
            with _lock:
                _runtime_state.setdefault("executions", {})[post_load_execution_id] = dict(post_load)
                _runtime_state["last_execution_id"] = post_load_execution_id
                _runtime_state["last_agent_action"] = f"DEV post-load registered for {migration_id}: {total_loaded}/{expected_ready} loaded; {review_held} review held"
            _persist_execution_state(post_load)
            return self._send(200, envelope("post_load_execution", post_load))
        ...
        if path == "/v1/tests":
            ...
            execution_status = str(execution.get("status", "")).upper()
            is_post_load = execution_status == "POST_LOAD_COMPLETED"
            is_dry_run = execution_status == "DRY_RUN_COMPLETED"
            loaded_count = int(execution.get("loaded_record_count", 0) or 0)
            ...
            checks = [
                {
                    ...
                    "critical": True,
                    "passed": (
                        is_post_load or is_dry_run
                    ),
                },
                {
                    "id": "TST-002",
                    "name": "Execution mode is governed DEV",
                    "critical": True,
                    "passed": (
                        str(execution.get("environment", "")).upper() == "DEV"
                        and str(execution.get("execution_mode", "")).upper() in {"DRY_RUN", "DEV_REPLACE_LOAD"}
                    ),
                },
                {
                    "id": "TST-003",
                ...
                },
                {
                    ...
                    "passed": (
                        input_count > 0
                        and ((is_post_load and input_count == loaded_count == len(ready_records))
                             or (is_dry_run and input_count == simulated_count == len(record_results)))
                    ),
                },
                {
                    "id": "TST-005",
                    "name": "Governed execution completed without record failures",
                    "critical": True,
                    "passed": (
                        failure_count == 0
                        and success_count == input_count
                        and ((is_post_load and loaded_count == input_count)
                             or (is_dry_run and all_records_simulated and simulated_count == input_count))
                    ),
                },
                {
                    "id": "TST-006",
                    "name": "DEV target-write policy and production safety are correct",
                    "critical": True,
                    "passed": (
                        execution.get("production_action_executed") is not True
                        and execution.get("production_executed") is not True
                        and production_action_count == 0
                        and ((is_post_load and execution.get("target_write_executed") is True and loaded_count == input_count)
                             or (is_dry_run and execution.get("target_write_executed") is not True and target_write_count == 0))
                    ),
                },
                {
                    "id": "TST-007",
                    "name": "Staging dispositions are preserved",
                    "critical": True,
                    "passed": (
                        len(ready_records) == input_count
                        and execution.get("review_records_written") is not True
                        and execution.get("quarantine_records_written") is not True
                        and execution.get("rejected_records_written") is not True
                    ),
                },
                {
                    "id": "TST-008",
            ...
            test_result = {
                ...
                "safety": {
                    "source_write_executed": execution.get("source_write_executed") is True,
                    "target_write_executed": execution.get("target_write_executed") is True,
                    "production_action_executed": execution.get("production_action_executed") is True,
                },
                "created_at": utc_now(),
            }
        ...
        if path == "/v1/migrations":
            ...
            with _lock:
                ...
                # DEV_DRY_RUN_001: dry-run is a simulation gate, not a target-write gate.
                # Review/quarantine/rejected/blocked dispositions and validation warnings
                # remain visible in the simulation and do not prevent a DEV-only dry run.
                # The approval snapshot still has to match authoritative evidence exactly.
                # Real target writes remain a separate later action and only consume
                # authoritative ready_records after a successful dry run.
            ...
            dry_run_gate = (
                ...
                if (
                    validation_ready
                    and held_record_count == 0
                    and runtime_blocking_findings == 0
                )
                else "PASS_WITH_HELD_RECORDS"
            )
            ...
            execution = {
                ...
                "execution_gate": dry_run_gate,
                "approval_status": "APPROVED",
                "preflight_validation_ready": validation_ready,
                "preflight_blocking_findings": runtime_blocking_findings,
                "environment": "DEV",
                ...
                "disposition_summary": {
                    ...
                    "ready_records": runtime_ready,
                    "review_records": runtime_review,
                    "blocked_records": runtime_blocked,
                    "quarantine_records": runtime_quarantine,
                    "rejected_records": runtime_rejected,
                    "held_records": held_record_count,
                    "target_write_eligible_records": runtime_ready,
                },
                "held_records_preserved": True,
                "review_records_written": False,
                "quarantine_records_written": False,
            ...
            _persist_execution_state(execution)
            payload = execution
        if path == "/v1/reconciliations":
            ...
            stored_execution = _get_authoritative_execution(execution_id)
            ...
            is_post_load_reconciliation = execution_status == "POST_LOAD_COMPLETED"
            loaded_record_count = int(stored_execution.get("loaded_record_count", 0) or 0)
            if is_post_load_reconciliation:
                simulated_record_count = loaded_record_count
                matched_records = loaded_record_count if int(stored_execution.get("failure_count", 0) or 0) == 0 else 0
                failed_records = max(input_record_count - matched_records, 0)
                unmatched_source = max(input_record_count - loaded_record_count, 0)
                unexpected_simulated = max(loaded_record_count - input_record_count, 0)
                count_variance = loaded_record_count - input_record_count
                target_write_count = loaded_record_count
            else:
                target_write_count = sum(
                    1 for item in record_results if item.get("target_write_executed") is True
                )
            ...
            reconciliation_pass = (
                ...
                and (target_write_count == input_record_count if is_post_load_reconciliation else target_write_count == 0)
                and production_action_count == 0
            )
            ...
            reconciliation = {
                ...
                "production_action_executed":
                    False,
                "target_write_executed":
                    is_post_load_reconciliation,
                "reconciliation_mode":
                    ("POST_LOAD_DEV" if is_post_load_reconciliation else "DRY_RUN"),
                "held_records": stored_execution.get("disposition_summary", {}),
            }
            ...
            _persist_reconciliation_state(reconciliation)
            if current_execution is not None:
                _persist_execution_state(current_execution)
        ...
        if path == "/v1/evidence":
            ...
            evidence_package = {
                ...
                "safety": {
                    ...
                    "production_action_executed":
                        False,
                    "target_write_executed":
                        reconciliation.get("target_write_executed") is True,
                    "target_write_count":
                        reconciliation.get(
                            "target_write_count",
                            0,
            ...
            _persist_evidence_state(evidence_package)
        ...
        if path == "/v1/approvals":
            migration_id = str(data.get("migration_id", "")).strip()
            if not migration_id:
                return self._send(400, envelope("approval_request_error", {
                    "message": "migration_id is required",
                    "authoritative": True,
                }))
            if str(data.get("environment", "")).upper() != "DEV":
                return self._send(400, envelope("approval_request_error", {
                    "message": "APPROVAL_CONTEXT_001 permits this UI approval workflow for DEV only",
                    "authoritative": True,
                }))
            if data.get("execution_requested") is True or data.get("target_write_requested") is True:
                return self._send(400, envelope("approval_request_error", {
                    "message": "Approval request may authorize DEV dry-run eligibility only; execution/target-write requests are forbidden",
                    "authoritative": True,
                }))

            approval_id = str(uuid.uuid4())
            _supersede_existing_approvals(migration_id)
            payload = {
                "id": approval_id,
                "migration_id": migration_id,
                "status": "PENDING",
                ...
                "target_write_executed": False,
                "input": data,
                "persistence_version": "APPROVAL_CONTEXT_001",
            }
            ...
            _persist_approval_state(payload)
...
# BEGIN KMITORA_A000_AGENT_CAPABILITY_LEARNING_011
try:
...
# END KMITORA_A000_AGENT_CAPABILITY_LEARNING_011
