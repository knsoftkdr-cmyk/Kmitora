...
import TestQualificationProgress from "../components/TestQualificationProgress";
import {
  AlertTriangle,
  ArrowRight,
  CheckCircle2,
...
import { getLatestExecutionForMigration, postTestExecution } from "../services/api";
...
type ExecutionResult = {
  ...
  parent_dry_run_execution_id?: string;
...
export default function Test({
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      const storedTest = readStoredJson<TestResult>(TEST_KEY);
      const currentMigrationId =
        readStoredJson<{ migration_id?: string }>(DISCOVERY_KEY)?.migration_id ?? "";

      let activeExecution: ExecutionResult | null = null;
      if (currentMigrationId) {
        try {
          activeExecution = await getLatestExecutionForMigration(currentMigrationId) as ExecutionResult | null;
        } catch (executionLookupError) {
          console.warn("KMITORA Test authoritative execution lookup failed", executionLookupError);
        }
      }
      ...
      if (activeExecution) {
        try {
          localStorage.setItem(EXECUTION_KEY, JSON.stringify({
            execution_id: activeExecution.execution_id,
            migration_id: activeExecution.migration_id,
            status: activeExecution.status,
            execution_mode: activeExecution.execution_mode,
            environment: activeExecution.environment,
            input_record_count: activeExecution.input_record_count,
            simulated_record_count: activeExecution.simulated_record_count,
            loaded_record_count: activeExecution.loaded_record_count,
            success_count: activeExecution.success_count,
            failure_count: activeExecution.failure_count,
            target_write_executed: activeExecution.target_write_executed,
      ...
      if (
        ...
        storedTest.execution_id === activeExecution.execution_id
      ) {
  ...
  const executionMode = String(execution?.execution_mode ?? "").toUpperCase();
  const isPostLoad = executionStatus === "POST_LOAD_COMPLETED";
  const executionEligible =
    Boolean(execution?.execution_id) &&
    isPostLoad &&
    executionMode === "DEV_REPLACE_LOAD" &&
    String(execution?.environment ?? "DEV").toUpperCase() === "DEV" &&
  ...
  return (
    ...
    >
      ...
      {!executionEligible && execution && (
        <div style={{ marginTop: 14, padding: 12, border: "1px solid #f0c36d", borderRadius: 10, background: "#fffaf0" }}>
          Test is gated on the authoritative POST_LOAD_COMPLETED DEV execution. The earlier dry-run execution is not sufficient after real target writes.
        </div>
      )}
      ...
      >
        ...
        <TestQualificationProgress executionStatus={executionStatus} />
