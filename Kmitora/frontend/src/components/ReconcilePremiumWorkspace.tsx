...
export default function ReconcilePremiumWorkspace() {
  ...
  const snapshot = useMemo(() => {
    ...
    const recordResultCount = numberOf(
      evidence?.record_result_count ?? recordResults.length
    );
    const transformationEvidenceCount = numberOf(
      evidence?.transformation_evidence_count ?? transformationEvidence.length
    );
    const businessRuleCount = numberOf(
      evidence?.business_rule_count ?? businessRules.length
    );
    ...
    const isPostLoad =
      String(result?.reconciliation_mode ?? "").toUpperCase() === "POST_LOAD_DEV";
    ...
    const invariantPreserved =
      ...
      (isPostLoad
        ? result?.target_write_executed === true && targetWrites === matched
        : targetWrites === 0 && result?.target_write_executed !== true);
    ...
    <div className="reconcilePremium" ref={rootRef}>
      <header className="reconcilePremiumHeader">
        <div>
          ...
          <p>
            Compare authoritative source-ready records with the actual DEV target result, mappings and transformation evidence from the same governed KMITORA DEV execution.
          </p>
          <div className="reconcileSafetyRow">
            <span><ShieldCheck size={13}/>Governed DEV verification</span>
            <span><Database size={13}/>Target writes: {snapshot.targetWrites}</span>
            <span><ShieldCheck size={13}/>DEV write boundary: {snapshot.invariantPreserved ? "PRESERVED" : "REVIEW"}</span>
      ...
      <nav className="reconcileTabs" aria-label="Reconciliation workspace">
        {([
          ...
          ["WORKBENCH", snapshot.isPostLoad ? "Reconcile DEV Post-Load" : "Reconcile DEV Dry-Run", GitCompareArrows],
          ["SOURCE_TARGET", "Source vs Target", Database],
          ["MAPPING", `Mappings & Transformations (${snapshot.transformationEvidenceCount})`, GitCompareArrows],
          ["VARIANCE", `Variance Analysis (${snapshot.mismatch})`, AlertTriangle],
          ["EVIDENCE", `Evidence (${snapshot.recordResultCount})`, FileCheck2],
          ...
          <div className="reconcileTopGrid">
            <section>
              <div className="reconcilePanelTitle">
                <div>
                  <span>{snapshot.isPostLoad ? "RECONCILE DEV POST-LOAD" : "RECONCILE DEV DRY-RUN"}</span>
      ...
      {view === "INTELLIGENCE" && (
        <div className="reconcileIntelligence">
          ...
          <div className="reconcileIntelCards">
            ...
            <article><ShieldCheck size={18}/><span>Safety Invariant</span><strong>{snapshot.invariantPreserved ? "PRESERVED" : "REVIEW"}</strong><small>DEV target writes are permitted only for POST_LOAD_DEV; production actions remain zero</small></article>
          </div>
