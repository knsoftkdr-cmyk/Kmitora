...
const ports: Record<TargetType, number> = {
  postgresql: 5432,
  oracle: 1521,
  sqlserver: 1433,
  mysql: 3306,
...
export default function KMITORATargetConnectWorkspace({ adapter }: { adapter: TargetConnectAdapter }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const canonical = listed.find((x) => x.id === created.id) || created; localStorage.setItem("kmitora.active.targetId", canonical.id); void fetch("/api/v1/ui-state", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active_target_id: canonical.id }) }).catch((error) => console.warn("KMITORA target selection persistence failed", error)); localStorage.removeItem("kmitora.discovery.unified.v1"); localStorage.removeItem("kmitora.dev.discoveryResult"); localStorage.removeItem("kmitora.dev.migrationId");
  ...
  const openTarget = async (target: TargetConnection) => { const previousActiveTarget=localStorage.getItem("kmitora.active.targetId"); localStorage.setItem("kmitora.active.targetId", target.id); void fetch("/api/v1/ui-state", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ active_target_id: target.id }) }).catch((error) => console.warn("KMITORA target selection persistence failed", error)); if(previousActiveTarget && previousActiveTarget!==target.id){ localStorage.removeItem("kmitora.discovery.unified.v1"); localStorage.removeItem("kmitora.dev.discoveryResult"); localStorage.removeItem("kmitora.dev.migrationId"); } setSelectedTarget(target); showTargetDetails(target); await loadObjects(target); };
    ...
    <div className="kt-grid">
      <div className="kt-card">
        ...
        <div className="kt-form-grid">
          ...
          <label>Target Type<select disabled={formMode === "view"} value={draft.type} onChange={(e)=>updateType(e.target.value as TargetType)}><option value="postgresql">PostgreSQL</option>
...
<option value="other">Other / Custom Database</option></select></label>
