...
export default function KMITORABusinessRequirementsWorkspace({ onClose, onContinue }: Props) {
  ...
  const saveRequirement = () => {
    ...
    // PERSISTENT-UI-STATE-001: persist requirement server-side so a browser or
    // service restart cannot erase the active DEV migration context.
    void fetch("/api/v1/ui-state", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ business_requirement: record }),
    }).catch((error) => console.warn("KMITORA requirement persistence failed", error));
